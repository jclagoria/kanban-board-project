...
const errorMsgCss: React.CSSProperties = {
  fontSize: 12, color: '#c00', marginTop: 4, position: 'relative',
}
const colorSwatchCss: React.CSSProperties = { width: 24, height: 24, border: '1px solid #aaa', background: '#d4e5f7', cursor: 'pointer' }
...
export default function CreationFormValidationError({ showStructure }: { showStructure?: boolean }) {
  return (
    <div style={{ position: 'relative' }}>
      {showStructure && <RegionLabel top={0} left={0}>Modal — Creation Form (Validation Error State)</RegionLabel>}
      <div style={overlayCss}>
        ...
        <div style={bodyCss}>
          <div style={fieldCss}>
            ...
            <input style={inputErrorCss} type="text" defaultValue="AB" />
            {showStructure && <RegionLabel top={24} left={0}>Validation Error — Inline</RegionLabel>}
            <div style={errorMsgCss}>Name must be between 5 and 50 characters.</div>
          </div>

          <div style={fieldCss}>
            <label style={labelCss}>Description <span style={hintCss}>(optional, max 500 chars)</span></label>
            <textarea style={textareaCss} placeholder="Describe the board purpose" />
          </div>
