...
export default function ValidationRequiredEmpty({ showStructure }: { showStructure?: boolean }) {
  return (
    <div style={{ position: 'relative' }}>
      {showStructure && <RegionLabel top={0} left={0}>Validation Error — Required Field Empty on Blur</RegionLabel>}
      <div style={paneCss}>
        <div style={headerCss}>
        ...
        <div style={sectionCss}>
          ...
          <div style={fieldRowCss}>
            {showStructure && <RegionLabel top={-4} left={0}>CF: Number — required empty [!]</RegionLabel>}
            <label style={labelCss}>
              Story Points<span style={requiredStarCss}> *</span>
              <span style={optionalTagCss}>(Required)</span>
            </label>
            <div style={{ position: 'relative' }}>
              <input style={inputErrorCss} type="number" value="" placeholder="Enter story points" />
              <span style={errorIconCss}>!</span>
            </div>
            <div style={errorBoxCss}>
              {showStructure && <RegionLabel top={-2} left={0}>Error — Required field empty</RegionLabel>}
              ⚠ This field is required.
            </div>
          </div>

          <div style={fieldRowCss}>
            {showStructure && <RegionLabel top={-4} left={0}>CF: Long Text — valid reference</RegionLabel>}
            <label style={labelCss}>
              Notes
